...
Invoke the `tasks-manager` skill and create a new per-task journal.
...
2. Create `./.claude-memory/tasks/<slug>.md` using the per-task template defined in the `tasks-manager` skill (frontmatter + Goal, Related Documents, Key Findings, Decisions, Open Questions, Next Steps).
