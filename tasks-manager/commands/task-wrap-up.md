...
5. Check Cross-task findings: if `.claude-memory/research/` exists, promote overlapping findings there (researcher owns verified facts) and link; otherwise fall back to `./.claude-memory/findings/<topic>.md` (with `summary:` in frontmatter).
