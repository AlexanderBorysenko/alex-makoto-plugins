---
description: Run the memory system startup ritual now — load architecture cache and the open-task index (no task auto-loaded).
---

Invoke the `memory-system` skill and execute the **Startup ritual** section against this project:
...
Do not load architecture component detail pages or any task journal until a task is in focus. Stay within the startup token budget.
